...
namespace ApiForTestOHIF.Services
{
    ...
    public class XmlDicomConverter : DicomConverterBase, IXmlDicomConverter
    {
        ...
        {
            IByteBuffer buffer = GetItemBuffer(item);

            if (buffer is IBulkDataUriByteBuffer)
            {
                ...
                writer.WriteAttributeString(Constants.ATTRIBUTE_BULKDATAURI, ((IBulkDataUriByteBuffer)buffer).BulkDataUri);
                writer.WriteEndElement();
            }
            else
            {
        ...
        )
        {
            ...
            else if (Utilities.IsBinaryVR(dicomVr))
            {
                ...
                if (null != dataElement)
                {
                    ...
                    {
                        ds.AddOrUpdate<IByteBuffer>(dicomVr, tag, data);
                    }
                }
            }
            else
